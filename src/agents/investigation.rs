...
use crate::models::{Message, PriceHistory, StockAction};
use anyhow::Result;
    ...
    async fn process(
        ...
        if requires_discovery && !discovery_topic.is_empty() {
            self.base.add_trace(&format!(
                "Buscando nuevos activos para el tema: {}",
                discovery_topic
            ));
            let disc_query = format!(
                "Identify top 3-4 stock tickers or ETFs for {}",
                discovery_topic
            );
            ...
            let req = crate::ollama::ChatRequest {
                ...
                messages: vec![
                    crate::ollama::ChatMessage {
                        ...
                        content: format!(
                            "Basado en los resultados de búsqueda, identifica los 3-4 tickers más relevantes para {}. Devuelve SOLO un array JSON de strings con los tickers.",
                            discovery_topic
                        ),
                        images: None,
                    },
        ...
        for company in companies {
            let query = format!(
                "latest stock news and market sentiment for {} {}",
                company, company
            );
            ...
            let system_prompt = format!(
                "Analiza las noticias de {}. Genera un razonamiento con recomendaciones de compra/venta basado en el sentimiento actual del mercado en ESPAÑOL.\nDevuelve JSON: {{ \"company\": \"...\", \"ticker\": \"...\", \"reasoning\": \"...\" }}",
                company
            );
            let req = crate::ollama::ChatRequest {
                model: self.base.model.clone(),
            ...
            let mut stock_action: StockAction =
                crate::agents::parse_llm_json(&res.message.content)?;
...
impl Agent for StockDataAgent {
    ...
    async fn process(
        ...
        let mut actions: Vec<StockAction> =
            serde_json::from_value(context["investigative_results"].clone())?;

        for action in actions.iter_mut() {
            let query = format!(
                "{} stock historical prices today vs 1 week ago vs 1 year ago at google finance bloomberg",
                action.ticker
            );
            let search_res = self.base.client.web_search(query).await?;
            ...
            let sys_prompt = format!(
                "Extrae precios numéricos precisos para {}. Un precio para hoy, un precio de hace 7 días y un precio de hace 1 año.\nDevuelve JSON: {{ \"today\": 0.0, \"one_week\": 0.0, \"one_year\": 0.0 }}",
                action.ticker
            );
            let req = crate::ollama::ChatRequest {
                model: self.base.model.clone(),
